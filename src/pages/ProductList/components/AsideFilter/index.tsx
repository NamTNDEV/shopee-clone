...
export default function AsideFilter({ categories, queryConfigs }: Props) {
  const {
    control,
    handleSubmit,
    trigger,
  ...
  const handleRemoveAll = () => {
    navigate({
      pathname: path.home,
      search: createSearchParams(
        _omit(queryConfigs, ['price_min', 'price_max', 'rating_filter', 'category'])
      ).toString()
    })
  }
    ...
    <div className='py-4'>
      <Link
        ...
        className={classNames('flex items-center font-bold', {
          'text-orange-600': !queryConfigs.category
        })}
      >
      ...
      <ul>
        {categories.map((categoryItem) => {
          const isActive = queryConfigs.category === categoryItem._id
          return (
      ...
      <RatingStars queryConfig={queryConfigs} />
      <div className='bg-gray-300 h-[1px] my-4' />
      <Button
